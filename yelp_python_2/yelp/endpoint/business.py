...
from yelp.config import BUSINESS_PATH, SEARCH_PATH, REVIEW_PATH
...
class BusinessEndpoints(object):
    ...
    def get_review_by_id(self, business_id, **url_params):
        review_path = REVIEW_PATH.format(id=business_id)
        response = self.client._make_request(review_path, url_params=url_params)
        return response
